    ...
    <div className='filtro-buscar-form'>
      ...
      (objetos.map((objeto, index) =>
              ...
              (<ResultadoCard key={index} tipo={tipo} nome={objeto.nome} idObjeto={objeto.id} dados={["Questoes: " + objeto.quantidadeQuestoes, "Realizações: " + objeto.popularidade]}/>) 
               :
               (tipo == 3) ?
              (<ResultadoCard key={index} tipo={tipo} nome={objeto.nomeProva} idObjeto={objeto.id} dados={[
              "Corrigida Totalmente: " + (objeto.corrigida? "Sim" : "Não")
              , objeto.corrigida? "Nota Final: " : "Nota Parcial: " + objeto.nota + "/" + objeto.notaMaxima,
              "Data: " + objeto.dataResolucao]}/>)
              :
              (<ResultadoCard key={index} tipo={tipo} nome={objeto.nomeAluno} idObjeto={objeto.id} dados={[
                "Corrigida Totalmente: " + (objeto.totalmenteCorrigida? "Sim" : "Não")
                , objeto.totalmenteCorrigida? "Nota Final: " : "Nota Parcial: " + objeto.notaAluno + "/" + objeto.notaMaxima,
                "Data: " + objeto.dataResolucao,
