...
export function obterProvasPorConteudo(pagina, nome, ordenacao, idConteudo) {
  return axios.get(
    `${url}/buscarPorConteudo?pagina=${pagina}&nome=${nome}&ordenacao=${ordenacao}&idConteudo=${idConteudo}`,
    { headers: AuthHeader() }
  );
}
...
export function obterProvaFazer(idProva) {
  return axios.get(`${url}/buscarFazerID?id=${idProva}`, {
    headers: AuthHeader(),
  });
}
...
export function obterProvasRealizadas() {
  return axios.get(`${url}/buscarResolucoesPorUsuario`, {
    headers: AuthHeader(),
  });
}
...
export function corrigirProva(body) {
  return axios.put(`${url}/corrigirDissertativa`, body, {
    headers: AuthHeader(),
  });
}
